// Since the i equals to the length of the array, the outer loop will run until i is greater than 0.
// ==> Because inner loop's process is dependent on outer loop, this is why we start i from end toward the start.
// ==> inner loop uses outer loop's pointer, which is i, that is equal to "arr.length" we that use to implement a condition that inner loop will run until i - 1. Becuase the last element is already in place. This helps us to reduce the unnecessary steps.
